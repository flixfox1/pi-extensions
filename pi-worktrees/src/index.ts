...
import { showCreateWizard, showWorktreePanel } from './ui/worktreePanel.ts';

const HELP_TEXT = `
/worktree - Git worktree management

Commands:
  /worktree                        Open compact-panel-style worktree dashboard
  /worktree panel                  Open compact-panel-style worktree dashboard
...
const commands: Record<string, CmdHandler> = {
  panel: async (_args, _ctx) => { /* handled before command dispatch */ },
  init: cmdInit,
  ...
  pi.registerCommand('worktree', {
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        const deps = {
          // current(ctx) requires a git repo. For the top-level panel we still want
          // to render a friendly "not in a git repository" card instead of failing
          // during dependency construction.
          settings: (() => {
            try { return configService.current(ctx); }
            catch { return configService.worktrees.get('**') ?? {}; }
          })(),
          configService,
          statusService,
        };

        if (!trimmed || cmd === 'panel') {
          await showWorktreePanel(pi, ctx, deps);
          return;
        }

        if (cmd === 'create' && rest.join(' ').trim() === '' && ctx.hasUI) {
