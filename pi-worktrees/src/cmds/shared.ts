...
export async function runHook(
  ...
  const emitProgress = (
    ...
    success?: boolean,
    failed?: HookProgressEvent['failed']
  ) => {
    options?.onProgress?.({
      hookName,
      worktree: createdCtx,
      ...
      outputs: commandOutputs.map((output) => ({ ...output })),
      currentIndex,
  ...
  for (const [index, command] of commands.entries()) {
    ...
    if (!result.success) {
      ...
      const failed = { command, code: result.code, error: result.stderr };
      emitProgress(index, true, false, failed);
      if (!options?.onProgress) {
        notify(
          formatCommandList(
            commands,
